...
#define I2C_BUS_DEV_PATH "/dev/i2c"
...
int i2c_hub_select_port(uint8_t hub_address, uint8_t port)
{
    ...
    if (ioctl(i2c_fd, I2C_SLAVE_FORCE, hub_address) < 0) {
        LE_ERROR("Could not set address to 0x%02x: %m\n", hub_address);
        close(i2c_fd);
        return -1;
    }
    int writeResult = i2c_smbus_write_byte(i2c_fd, port);
    if (writeResult < 0) {
        LE_ERROR("smbus write failed with error %m\n");
        result = -1;
    } else {
        result = 0;
    }
...
int I2C_write_bytes(uint8_t addr, uint8_t reg, uint8_t length, uint8_t* data)
{
    ...
    if (ioctl(i2c_fd, I2C_SLAVE_FORCE, addr) < 0) {
        LE_ERROR("Could not set address to 0x%02x: %m", addr);
        close(i2c_fd);
        return -1;
    }
...
void set_pwm(uint8_t pin, uint16_t led_on, uint16_t led_off)
{
    ...
    if (res < 0)
    {
        LE_ERROR("write bytes failed with error %m");
    }
}
...
LE_SHARED void servo_SetAngle(uint8_t pin, uint16_t angle)
{
...
COMPONENT_INIT
{
    LE_INFO("Initializing Grove PWM control.");
    servo_init();
}
